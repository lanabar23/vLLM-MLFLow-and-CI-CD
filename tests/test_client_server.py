...
def test_get_data(url):
    logging.debug(f"Отправляем GET-запрос на {url}/data")
    print('get')
    try:
...
def test_post_data(url):
    payload = {'field': 'value'}
    logging.debug(f"Отправляем POST-запрос на {url}/post-data с данными: {payload}")
    print('post')
    try:
        response = requests.post(f"{url}/post-data", json=payload, timeout=10)
        logging.debug(f"Получен ответ с кодом {response.status_code}")
